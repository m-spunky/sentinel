...
import { ShieldCheck, Zap, Globe, Activity, Mail, Search, MessageSquare, Target, Lock, Share2, MoreHorizontal, Radar, Database, Sparkles, Filter, ShieldAlert, XCircle, Info, LayoutDashboard } from "lucide-react"
...
export function ComparisonSection() {
  ...
  return (
    <section id="compare" className="relative py-16 md:py-20 lg:py-24 px-6 bg-black overflow-hidden border-t border-white/5">
      {/* 🔮 Background Glow */}
      <div className="absolute top-1/2 left-0 -translate-y-1/2 h-[600px] w-[600px] bg-accent/5 rounded-full blur-[140px] -z-10" />
      <div className="absolute top-[20%] right-[-10%] h-[500px] w-[500px] bg-accent/3 rounded-full blur-[120px] -z-1" />
      ...
      <div className="max-w-7xl mx-auto flex flex-col items-center">
        <div className="text-center max-w-4xl space-y-10 mb-16 z-10 animate-in fade-in slide-in-from-top-12 duration-1000">
           <div className="flex items-center justify-center space-x-3 text-accent group opacity-60 hover:opacity-100 transition-opacity uppercase font-black tracking-[0.4em] text-[10px]">
              <Radar className="h-4 w-4 fill-accent" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center w-full z-10">
           {/* Left: Comparison Table Grid */}
           <div className="w-full space-y-4 animate-in fade-in slide-in-from-left-12 duration-1000 delay-300">
              {tableData.map((row, i) => (
                <div key={i} className="group relative">
                   <div className="absolute -left-4 top-0 bottom-0 w-0.5 bg-accent/5 group-hover:bg-accent/40 group-hover:w-1 transition-all" />
                   
                   <div className="grid grid-cols-3 gap-8 py-8 px-6 rounded-2xl bg-[#070D14]/40 border border-white/5 group-hover:bg-[#1B263B]/20 group-hover:border-accent/10 transition-all duration-500">
                      <div className="flex flex-col space-y-1">
                         <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-60">Capability</span>
                         <span className="text-sm font-black text-foreground group-hover:text-accent transition-colors">{row.feature}</span>
                      </div>
                      ...
                      <div className="flex flex-col space-y-1 text-right">
                         <span className="text-[10px] font-black uppercase tracking-widest text-accent opacity-60">SentinelAI</span>
                         <span className="text-sm font-black text-accent">{row.sentinel}</span>
                      </div>
                   </div>
                </div>
              ))}
           ...
           <div className="space-y-10 animate-in fade-in slide-in-from-right-12 duration-1000 delay-500">
              ...
              <div className="grid grid-cols-2 gap-6">
                 {[
                   ...
                   <div key={i} className="card-cyber-compact p-8 space-y-4 relative group hover:scale-105 transition-all">
                      <div className="absolute top-4 right-4 h-1 w-8 bg-accent/5 group-hover:bg-accent/20 transition-all" />
                      <div className="h-10 w-10 rounded-xl bg-accent/10 flex items-center justify-center border border-accent/20 group-hover:border-accent/40 transition-all">
                         <kpi.icon className="h-5 w-5 text-accent" />
                      </div>
                      <div className="space-y-1">
                         <div className="text-3xl font-black tracking-tighter text-foreground group-hover:text-accent transition-colors">{kpi.reduction}</div>
              ...
              <div className="pt-6 border-t border-white/5">
                 <div className="flex items-center space-x-6">
                    <div className="flex -space-x-3">
                       {[1, 2, 3].map((_, i) => (
                         <div key={i} className="h-10 w-10 rounded-full bg-accent/20 border-2 border-black flex items-center justify-center text-[10px] font-bold text-accent">AI</div>
                       ))}
                    </div>
